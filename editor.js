...
const wizard = {
    init: () => {
        // Спочатку ініціалізуємо слухачі, потім рендеримо
        initButtons();
        initStep1Listeners();     // <-- ВИПРАВЛЕНО: Слухач для зміни кількості пар
        initSubjectFormListeners();
        initDropModal();
        setupGlobalListeners();
        
        // Завантажуємо дані та малюємо
        tryLoadFromStorage();
        renderTimeInputs();       // <-- ВИПРАВЛЕНО: Примусовий рендер часу
        updateUI();
    },
    next: () => {
        if (state.step === 1) {
...
function initStep1Listeners() {
    ...
    if(pairsSelect) {
        pairsSelect.addEventListener('change', renderTimeInputs);
    }
}
...
function renderTimeInputs() {
    ...
    if(!container || !pairsSelect) return;
    
    container.innerHTML = '';
    const n = parseInt(pairsSelect.value) || 5; // Default 5
    
    for(let i=0; i<n; i++) {
        // Беремо час зі state або дефолтний
        let val = "00:00 – 00:00";
        if (state.settings.times && state.settings.times[i]) {
            val = state.settings.times[i];
        }
        
        container.innerHTML += `<div><label>Пара ${i+1}</label><input class="time-in" type="text" value="${val}" name="time-${i}"></div>`;
    }
}
    ...
    checkboxes.forEach(cb => {
        ...
        const rowsHtml = variants.map((v, idx) => `
            <div class="variant-row" data-index="${idx}">
                <input type="text" class="inp-teacher" placeholder="Викладач" value="${v.teacher || ''}" style="flex:1" name="teacher-${type}-${idx}" id="teacher-${type}-${idx}" autocomplete="off">
                <input type="text" class="inp-room" placeholder="Ауд." value="${v.room || ''}" style="flex:0.5" name="room-${type}-${idx}" id="room-${type}-${idx}" autocomplete="off">
        ...
        const html = `
            <div class="type-detail-block" data-type="${type}">
                ...
                <div class="variants-list">${rowsHtml}</div>
            </div>
        `;
...
function changeGrid(action) {
    ...
    if (action === 'time') {
        ...
        const defaultTime = state.settings.times[pIndex] || "00:00";
...
function saveFinalResult() {
    ...
    dayKeys.forEach((dayKey, dIndex) => {
        ...
        for(let p=0; p<state.settings.pairsPerDay; p++) {
            ...
            // Якщо клітинка пуста (і не має структури) або порожній контент
            if (!cell || !cell.content || Object.keys(cell.content).length === 0) {
                // Додаємо пустий слот, щоб зберегти порядок пар
                lessons.push({
                    number: p + 1,
                    time: defaultTime,
                    type: "empty",
